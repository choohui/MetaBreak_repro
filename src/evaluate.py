"""MetaBreak TM-1 reproduction — Stage 4: judge jailbreak success.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model_type", required=True,
                   help="Family slug. Used for output annotation and "
                        "default paths (results/<model_type>/*).")
    p.add_argument("--responses", default=None,
                   help="Path to responses.jsonl. Default: "
                        "results/<model_type>/responses.jsonl.")
    p.add_argument("--output", default=None,
                   help="Path to eval_report.json. Default: "
                        "results/<model_type>/eval_report.json.")
    p.add_argument("--per_item", default=None,
                   help="Path to eval_per_item.jsonl. Default: "
                        "results/<model_type>/eval_per_item.jsonl.")
    p.add_argument("--guard_model", default=None,
                   help="Optional: local HF path to Llama-Guard-3-8B "
                        "(or Llama-Guard-2-8B). Skipped if omitted. The "
                        "guard is family-agnostic.")
...
def _result_dir(model_type: str) -> Path:
    return Path(__file__).resolve().parent.parent / "results" / model_type
