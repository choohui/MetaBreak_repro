...
from experiments_hc_6.config import DATA_DIR, REPO_ROOT, ExpConfig
...
def _mock_replacement(lm) -> dict:
    return {
        ...
        "best_triple_decoded": ["uj챠c챠ch", "mock_start", "mock_end"],
        "neighbor_rank": 1,
...
def _ensure_data_files(cfg: ExpConfig) -> dict:
    ...
    if missing:
        raise FileNotFoundError("missing hc6 data inputs: " + ", ".join(missing))
    return {
