...
from experiments_hc_6.core.mimicry import assistant_header_from_replacement
...
def _display_replacement(repl: dict) -> str:
    repl_strs = [str(s) for s in repl.get("best_triple_decoded", []) if str(s).strip()]
    return repl_strs[0] if repl_strs else "ujicich"
...
def _gen_c(n: int, repl: dict) -> list[dict]:
    display = _display_replacement(repl)
    out = []
    for topic in TOPICS:
...
def run(cfg: ExpConfig, lm=None) -> dict:
    ...
    def add(idx, source, variant, text, **extra):
        row = {
            ...
            "variant": variant,
            "text": text,
            "is_attack_prompt": variant in {"malicious_mimicry", "malicious_special"},
        }
        row.update(extra)
        rows.append(row)
    ...
    summary = {"stage": "01_build_prompts", "n_prompts": len(rows), "counts": counts}
    io.write_json(cfg.out_dir / "build_prompts_summary.json", summary)
