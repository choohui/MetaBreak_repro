...
def build_base_matrix(rows: list[dict], top_ks: list[int]) -> tuple[np.ndarray, list[str]]:
    ...
    baseline = np.asarray([
        [
            float(bool(r.get("is_exact_special_token"))),
            float(bool(r.get("is_known_replacement_token"))),
            float(bool(r.get("is_l2_neighbor_proxy"))),
        ]
        for r in rows
    ], dtype=np.float64)
    cols.append(baseline)
    names.extend(["exact_special_token", "known_replacement_token", "l2_neighbor_proxy"])
    for prefix, mat in {
        "sink": sink,
        "sink_rank_pct": rank,
        "value_norm": value,
...
def add_hidden_projection_features(x: np.ndarray, names: list[str], rows: list[dict], hidden: np.ndarray,
    ...
    for layer in range(h.shape[1]):
        ...
        anorm = np.linalg.norm(mu_a) + 1e-12
        hnorm = np.linalg.norm(ha, axis=1, keepdims=True) + 1e-12
        cos_to_attack = ((ha @ mu_a) / (hnorm[:, 0] * anorm)).reshape(-1, 1)
        ...
        feats.extend([proj, cos_to_attack, dist_b, maha_b])
        feat_names.extend([
            f"proto_attack_minus_benign_L{layer}",
            f"cos_to_attack_L{layer}",
