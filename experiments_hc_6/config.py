...
ALL_STAGES = [str(i).zfill(2) for i in range(13)]
...
class ExpConfig:
    ...
    run_name: str = "hc6_mask_steering"
    ...
    stress_n: int = 20
    mask_candidate_k: int = 64
    mask_top_n: int = 12
    steer_layers: str = "auto"
    steer_alphas: list[float] = field(default_factory=lambda: [0.0, 0.25, 0.5, 1.0, 2.0])
    steer_modes: list[str] = field(default_factory=lambda: ["add", "project_out", "pull_to_benign"])
    semantic_eval_n: int = 64
...
def add_common_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--run_name", default="hc6_mask_steering")
    ...
    p.add_argument("--mask_candidate_k", type=int, default=64)
    p.add_argument("--mask_top_n", type=int, default=12)
    p.add_argument("--steer_layers", default="auto")
    p.add_argument("--steer_alphas", default="0,0.25,0.5,1,2")
    p.add_argument("--steer_modes", default="add,project_out,pull_to_benign")
    p.add_argument("--semantic_eval_n", type=int, default=64)
...
def config_from_args(args: argparse.Namespace) -> ExpConfig:
    return ExpConfig(
        ...
        mask_candidate_k=args.mask_candidate_k,
        mask_top_n=args.mask_top_n,
        steer_layers=args.steer_layers,
        steer_alphas=_float_list(args.steer_alphas),
        steer_modes=_str_list(args.steer_modes),
        semantic_eval_n=args.semantic_eval_n,
    )
