...
from experiments_hc_4_claude.core import benign_gen, benign_inject
...
from experiments_hc_4_claude.core.mimicry import apply_mimicry, load_prompts  # noqa: E402
...
def build_rows(cfg: ExpConfig) -> list[dict]:
    ...
    def add(idx, source, variant, position_kind, text, slot_word=None, extra=None):
        row = {
    ...
    # C — benign carriers with the *exact* mimicry token id injected (token-identity
    #     control). Stored as carrier_head/tail + inject_token_ids; stage 03 splices
    #     the id at extract time (the decoded string does not re-tokenize to the
    #     attack id in natural context — see core.benign_inject / Main.md §7.1).
    for r in benign_inject.build_c_prompts(cfg, repl):
        add(r["id"], r["source"], "benign_mimicry", "natural", r["text"],
            extra={"carrier_head": r["carrier_head"],
                   "carrier_tail": r["carrier_tail"],
