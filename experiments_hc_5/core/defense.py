...
def _expand_detected_spans(input_ids: list[int], positions: set[int], tpl) -> set[int]:
    out = set(int(p) for p in positions)
    literal = find_literal_assistant_spans(input_ids, tpl)
    regular = find_regular_assistant_spans(input_ids, tpl)
    last_literal_start = max((s.start for s in literal), default=-1)
    spans = regular + [s for s in literal if s.start != last_literal_start]
    for span in spans:
        span_positions = set(range(int(span.start), int(span.start) + len(span.ids)))
        span_replace = {int(span.start) + int(off) for off in tpl.replace_positions}
        if out & (span_positions | span_replace):
            out.update(span_positions)
    return out
...
def clean_ids(input_ids: list[int], positions: set[int], action: str, tokenizer, tpl=None) -> list[int]:
    ...
    if action == "no_op":
        return list(input_ids)
    if action == "drop_detected_span" and tpl is not None:
        base_positions = _expand_detected_spans(input_ids, positions, tpl)
    else:
    ...
    if action.startswith("drop_token") or action == "drop_detected_span":
