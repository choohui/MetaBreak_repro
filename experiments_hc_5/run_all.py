...
from experiments_hc_5.config import ALL_STAGES, config_from_args, make_parser  # noqa: E402
from experiments_hc_5.core.model import get_model  # noqa: E402
...
STAGE_FILES = {
    ...
    "06": "06_sanitize_eval.py",
    "07": "07_counterfactual_eval.py",
    "08": "08_stress_controls.py",
    "09": "09_report.py",
}
NEEDS_MODEL = {"00", "02", "06"}


def load_stage(num: str):
    path = HERE / "stages" / STAGE_FILES[num]
    spec = importlib.util.spec_from_file_location(f"hc5_stage_{num}", path)
...
def run(cfg, lm=None) -> dict:
    ...
    if lm is None and (set(stages) & NEEDS_MODEL):
        lm = get_model(cfg)
    print(f"[hc5] out_dir={cfg.out_dir}")
    print(f"[hc5] stages={stages} smoke={cfg.smoke}")
    results = {}
    for num in tqdm(stages, desc="[hc5] stages", unit="stage"):
