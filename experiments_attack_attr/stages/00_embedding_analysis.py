"""Stage 00 (choan.md §1) — token-EMBEDDING-level special-vs-regular geometry.

choan.md §1 question: are the MetaBreak mimicry / special / regular tokens
separable from the token EMBEDDING alone? Conclusion: NO (norm/centroid AUC ≈
0.5) — which is why §2 has to look at the model's *internal* representation. This
stage confirms that with the norm / centroid-distance separability AUC.

It also REGENERATES the mimicry signature ``replacement.json`` into ``out_dir``
(self-containment): on a real model via ``src.embedding`` (the canonical
MetaBreak L2 search), or by copying the vendored ``data/replacement.json`` under
the mock. Downstream stages read it via ``cfg.replacement_path()``.

Outputs (under ``out_dir``):
    embedding_analysis.json   - norm/cosine/centroid stats + norm-separability AUC
    embedding_analysis.md     - human-readable summary
    replacement.json          - mimicry signature regenerated for this model
"""
...
from experiments_attack_attr.core import io
...
from experiments_attack_attr.core import metrics  # noqa: E402
...
def _write_replacement(cfg: ExpConfig, lm) -> None:
    """Regenerate the mimicry signature for THIS model into ``out_dir`` so the
    folder stays self-contained (no read of the shared results/llama copy)."""
    out = cfg.out_dir / "replacement.json"
    cfg.out_dir.mkdir(parents=True, exist_ok=True)
    if getattr(lm, "is_mock", False):
        # mock: the vendored Llama signature matches the mock tokenizer's ids.
        io.write_json(out, io.load_replacement(cfg.replacement))
        print(f"[00] mock replacement copied -> {out}")
        return
    from src.embedding import search_best_tuple
    from src.model_configs import resolve_config
    mcfg = resolve_config(cfg.model_type, lm.tokenizer)
    emb = lm.embedding if hasattr(lm.embedding, "shape") else None
    import torch
    emb_t = emb if isinstance(emb, torch.Tensor) else torch.as_tensor(np.asarray(emb))
    res = search_best_tuple(mcfg, lm.tokenizer, emb_t, topk=200, verbose=False)
    res["assistant_header"] = mcfg.assistant_header
    io.write_json(out, res)
    print(f"[00] regenerated replacement.json ({res['best_triple_decoded']}) -> {out}")


def run(cfg: ExpConfig, lm=None) -> dict:
    lm = get_model(cfg, lm)
    _write_replacement(cfg, lm)
