"""Stage 05 (choan.md §2.2) — reduce the internal representation to one scalar
SIGNAL per token, per layer (fit on TRAIN, score every row).

This is the §2.2 signal table: the clean① family (hidden_norm, value_norm,
output_norm, sink, active_value, active_output, cos_to_ref, and the
**cos_to_attack headline**) and the borderline② family (**diff_means**,
pca_sep_proj, lda_1d). For each pos_offset: split prompts into train/held-out,
fit every selected scalarizer's geometry on the TRAIN rows ONLY, reduce each
token to one scalar per layer, and persist the scores. Reports the HONEST
train-side AUC per (signal, layer) — out-of-fold for the fitted signals so the
stage-06 model selection is not optimistic — with a group-bootstrap CI.
...
from experiments_attack_attr.core import io, stats              # noqa: E402
...
def _run_offset(cfg: ExpConfig, off: int) -> dict:
    ...
    print(f"[05] pos{off}: {len(keys)} signals, eval_mode={eval_mode}; "
