...
def forward_capture_ids(lm: LoadedModel, input_ids: list[int]) -> ForwardCapture:
    """Like :func:`forward_capture` but for a pre-built ``input_ids`` sequence.

    Used to inject an exact token id into a benign carrier (category C) without a
    text round-trip — the BPE tokenizer does not reproduce the attack id from the
    decoded string in natural context. Mock path mirrors ``forward_capture``.
    """
    if getattr(lm, "is_mock", False):
...
def _run_capture(lm: LoadedModel, input_ids: torch.Tensor) -> ForwardCapture:
    """One forward pass over ``input_ids`` ([1, seq]) capturing all signals."""
