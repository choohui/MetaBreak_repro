...
def binary_metrics(
    ...
    thr_at: dict[float, float | None] = {t: None for t in fpr_targets}
    for t in thresholds:
        ...
        for target in fpr_targets:
            # highest TPR among thresholds meeting the FPR budget; record the
            # oriented threshold that achieves it (used as a cascade op-point).
    ...
    return {
        ...
        "threshold_at_fpr": {f"{int(t*100)}pct": thr_at[t] for t in fpr_targets},
    }
...
def probe_layer(x: np.ndarray, y: np.ndarray, folds: int = 5, seed: int = 0,
    ...
    ``groups`` (hc_2, limitation #3): when given, splits with **GroupKFold** so a
    prompt's tokens never straddle train/test — the honest, leakage-free AUC.
    Without it, the naive StratifiedKFold AUC (per-token shuffle) is reported.
    """
    ...
    if n_pos < 2 or n_neg < 2:
        return {"auc": float("nan"), "balanced_acc": float("nan"),
                "method": "none", "split": "none", "n_pos": n_pos, "n_neg": n_neg}

    use_groups = groups is not None and _HAVE_SKLEARN
    if use_groups:
        groups = np.asarray(groups)
        n_groups = len(np.unique(groups))
        k = min(folds, n_groups)
        if k < 2:                       # too few prompts to split by group
            use_groups = False
    if not use_groups:
        k = max(2, min(folds, n_pos, n_neg))
    split = "group" if use_groups else "stratified"
    ...
    if _HAVE_SKLEARN:
        ...
        for tr, te in splitter:
            if y[tr].min() == y[tr].max():     # degenerate train fold
                continue
        ...
        bacc = float(np.nanmean(baccs)) if baccs else float("nan")
        return {"auc": round(auc, 5), "balanced_acc": round(bacc, 5),
                "method": "logreg", "split": split, "folds": k,
                "n_pos": n_pos, "n_neg": n_neg}

    # Fallback: nearest-centroid in standardized space (no sklearn).
    aucs, baccs = [], []
    ...
    return {"auc": round(float(np.nanmean(aucs)), 5),
            ...
            "method": "nearest_centroid", "split": split, "folds": k,
            "n_pos": n_pos, "n_neg": n_neg}
