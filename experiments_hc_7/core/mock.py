...
def mock_forward_capture(lm: LoadedModel, text: str) -> ForwardCapture:
    ...
    return _mock_fabricate(lm, list(int(t) for t in ids),
                           zlib.adler32(text.encode("utf-8")) & 0x7FFFFFFF)


def mock_forward_capture_ids(lm: LoadedModel, input_ids) -> ForwardCapture:
    """Faithful id-level mock: fabricate directly from the given ids (no re-template),
    mirroring the real ``forward_capture_ids`` so the C-injection path is exercised."""
    ids = [int(t) for t in input_ids]
    seed = zlib.adler32((",".join(map(str, ids))).encode("utf-8")) & 0x7FFFFFFF
    return _mock_fabricate(lm, ids, seed)


def _mock_fabricate(lm: LoadedModel, ids: list[int], seed: int) -> ForwardCapture:
    ...
    return ForwardCapture(list(ids), hidden_states, attentions,
