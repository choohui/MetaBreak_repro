...
from experiments_hc_4.core.model import get_model  # noqa: E402

STAGE_FILES = {
    "00": "00_embedding.py",
    "01": "01_build_prompts.py",
    "02": "02_capture.py",
    "03": "03_balance_split.py",
    "04": "04_scalar_discovery.py",
    "05": "05_threshold_rules.py",
    "06": "06_apply_defense.py",
...
def load_stage(num: str):
    ...
    spec = importlib.util.spec_from_file_location(f"hc4_stage_{num}", path)
...
def run(cfg, lm=None) -> dict:
    ...
    if lm is None and (set(stages) & NEEDS_MODEL):
        lm = get_model(cfg)
    print(f"[hc4] out_dir={cfg.out_dir}")
    print(f"[hc4] stages={stages} smoke={cfg.smoke}")
    results = {}
    for num in tqdm(stages, desc="[hc4] stages", unit="stage"):
